...
def inverse_kinematics_numeric(target_pos, initial_guess=[0, 0, 0], max_iter=1400, lr=0.1, tol=0.1): #수치미분법
    theta = np.array(initial_guess, dtype=float)
    origin = np.array([0,0,105.5])

    x = target_pos[0]
    y = target_pos[1]
    z = target_pos[2]

    theta1 = np.rad2deg(np.arctan(y/x))
    ...
    if theta1 != None:
        for i in range(max_iter):
            ...
            point1 = np.array([140*np.cos(theta2),140*np.sin(theta2)])
            point2 = np.array([np.sqrt(x**2 + y**2),z-origin[2]])
            distance1 = np.linalg.norm(point1 - point2)
            error = np.abs(distance1 - 140)
            # print(theta2)
    ...
    elif flag_theta2 != False:
        theta_3_DH = np.rad2deg(np.arctan(abs(point1[0]-point2[0])/abs(point1[1]-point2[1])))
        print("ssss",theta_3_DH)
        theta3 = 180 - (90 - theta2_deg) - theta_3_DH
    
        theta[0] = theta1
        theta[1] = -(90-theta2_deg)
        theta[2] = theta3 - 90
...
if __name__ == "__main__":
    ...
    circle_points = np.array(circle_points).astype(float)
    ...
    for j in range(len(theta)):
        ...
        print(circle_points[j])
        thetas, success = inverse_kinematics_numeric(circle_points[j])
        print("\n결과:")
